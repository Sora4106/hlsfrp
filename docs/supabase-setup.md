...
最新版也會加入 `original_size_bytes` 與重複圖片唯一索引。之後只有「原始檔名及原始檔案大小都相同」時才會阻止再次上傳；同名但大小不同的圖片仍可加入。舊版已上傳圖片若沒有原始大小紀錄，管理工具會以轉換後的 WebP 大小作相容比對。
...
- 再次上傳相同檔名與相同大小的圖片，確認該張被略過；同批其他不同圖片仍應正常上傳。
- 在產品、分類或據點按「從圖片庫選擇」，確認可搜尋並套用既有圖片。
