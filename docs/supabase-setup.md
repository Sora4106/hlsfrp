...
如果先前已執行過舊版 `schema.sql`，請再執行一次最新版；它會新增產品影片欄位、`hls_media` 媒體索引與 Storage 權限，不會刪除既有內容。

若目前資料庫已在使用中，也可以只執行 `supabase/enable-video-media.sql`，它只會升級產品影片欄位、媒體格式與檔案大小限制，不會修改既有內容。

目前官網已改為「電子郵件自行聯絡」模式，前台不再蒐集或寫入詢問資料。已使用舊版架構的專案，請在 Supabase SQL Editor 額外執行 `supabase/disable-public-inquiry-storage.sql`，以停用匿名者對 `hls_inquiries` 的新增權限；這個檔案不會刪除既有的歷史詢問紀錄。

最新版也會加入 `original_size_bytes` 與重複媒體唯一索引。之後只有「原始檔名及原始檔案大小都相同」時才會阻止再次上傳；同名但大小不同的圖片或影片仍可加入。舊版已上傳圖片若沒有原始大小紀錄，管理工具會以轉換後的 WebP 大小作相容比對。
...
- 在「媒體庫」上傳一張 JPG 或 PNG，確認可預覽、複製網址，並可在產品的圖片欄位直接選圖上傳。
- 在產品的「產品影片」欄位上傳一支 MP4 或 WebM，儲存並發布後，確認公開產品頁可播放影片。
- 再次上傳相同檔名與相同大小的圖片或影片，確認該檔被略過；同批其他不同媒體仍應正常上傳。
- 在產品、分類或據點按「從媒體庫選擇」，確認可搜尋並套用既有圖片；在產品影片欄位開啟媒體庫時只會顯示影片。
- 開啟前台聯絡頁，確認沒有姓名、信箱、電話或需求填寫欄位，且「開啟電子郵件」只會啟動使用者自己的郵件程式。
- 執行 `disable-public-inquiry-storage.sql` 後，確認匿名使用者無法新增 `hls_inquiries`；管理者仍可在管理頁查看與整理既有歷史紀錄。
